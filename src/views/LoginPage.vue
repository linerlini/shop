  ...
  <div class="login">
    <van-form @submit="handleValidateCodeShow">
      <van-cell-group inset>
        <van-field v-model="account" name="account" label="账号" placeholder="请填写账号" :rules="[{ pattern: accountReg, required: true, message: '请填写账号,由6-10位数字组成' }]" />
        <van-field
          v-model="password"
          type="password"
          name="password"
...
import { requestLogin } from 'server/user'
...
const handleValidateCodeSuccess = async () => {
  ...
  const result = await requestLogin(account.value, password.value)
